...
from preprocessing.transforms import load_cached_image, load_cached_mask, DEFAULT_SIZE #pulls in Faisal's preprocessing functions for loading images and masks, and the default size of 512x512
...
class AttributeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = load_cached_image(self.data_root / "images" / f"{image_id}.png") #builds the cached image path (build_cache.py saves cached images as .png) and calls load_cached_image, which just opens+normalises since the cache is already resized to 512x512 - no size argument needed
        ...
        for attribute in self.attributes: #loops over the 5 attribute names
            ...
            if mask_path.exists():
                ...
                mask = torch.from_numpy(load_cached_mask(mask_path)).float() #load_cached_mask only takes a path - the cache is already resized, so no size argument
