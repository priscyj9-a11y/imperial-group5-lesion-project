...
from attribute_detection_v3.task2_eval.mask_io import (
    ATTRIBUTE_NAMES,
    ground_truth_mask_path,
    prediction_mask_path,
)
from attribute_detection_v3.task2_eval.metrics import all_metrics
...
def print_attribute_summary(rows: list[dict[str, object]]) -> None:
    ...
    for attribute in ATTRIBUTE_NAMES:
        ...
        mean_dice = np.nanmean([float(row["dice"]) for row in attr_rows])
        mean_iou = np.nanmean([float(row["iou"]) for row in attr_rows])
        mean_precision = np.nanmean([float(row["precision"]) for row in attr_rows])
        mean_recall = np.nanmean([float(row["recall"]) for row in attr_rows])
