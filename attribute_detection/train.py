...
from attribute_detection.dataset import AttributeDataset
from attribute_detection.loss import DiceBCELoss, dice_score
...
def parse_args() -> argparse.Namespace: #Creates an argument parser: turns text we type after python3 -m attribute_detection.train into actual Python values.
...
def build_train_dataset(data_root: Path, splits_csv: Path):
    ...
    # Oversample images with rare attributes (negative_network, streaks) by duplicating them
    # This gives rare attributes more training examples without needing more data
    rare_attributes = ["negative_network", "streaks"]  # Present in <10% of images
...
def main() -> None:
    ...
    for epoch in range(args.epochs): #loop over epochs
        ...
        #Early stopping: stop if val_loss hasn't improved for N epochs
        if args.early_stop_patience > 0:
            ...
            if early_stop_counter >= args.early_stop_patience:
                print(f"Early stopping: val_loss hasn't improved for {args.early_stop_patience} epochs. Stopping training.")
                break
